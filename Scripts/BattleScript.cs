...
public class BattleScript : MonoBehaviourPun
{
    ...
    {
        if(coli.CompareTag("Wizard"))
        {
            /* butt.SetActive(true);
            float _damageAmount=50f;
           ...
           float _damageAmount=10f;
           float sv=0f;
            if(spinnerScript.spinSpeed<3600)
            {
                spinnerScript.spinSpeed += _damageAmount;
            currentSpinSpeed = spinnerScript.spinSpeed;
            }
            photonView.RPC("DoDamage",RpcTarget.AllBuffered,sv);
